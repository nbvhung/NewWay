...
export function EditSubmissionModal({ open, onClose, editForm, setEditForm, allShippingLines, saving, onSave, submission, userRole }: Props) {
  ...
  return (
    ...
    >
      <div className="mb-4">
        <label className="text-xs font-medium text-[#94a3b8] mb-1.5 block">Kế hoạch <span className="text-red-500">*</span></label>
        <div className="flex flex-col gap-1.5 max-h-[180px] overflow-y-auto">
          {allShippingLines.map(sl => (
            <label key={sl.id}
              className={`flex items-center gap-2.5 px-3 py-2 bg-[#1e293b] border rounded-lg cursor-pointer text-xs transition-all ${
      ...
      <div className="grid grid-cols-2 gap-3">
        <div><label className="text-[10px] font-medium text-[#94a3b8] mb-1 block">Hàng 20</label>
        ...
        <div><label className="text-[10px] font-medium text-[#94a3b8] mb-1 block">Hàng 40</label>
          <input type="number" value={editForm.hang40 || ''} onChange={e => setEditForm({ ...editForm, hang40: e.target.value })}
            className="w-full px-3 py-2 bg-[#1e293b] border border-[rgba(255,255,255,0.08)] rounded-lg text-xs text-[#f1f5f9] outline-none focus:border-[#1a56db]" /></div>
        <div><label className="text-[10px] font-medium text-[#94a3b8] mb-1 block">Vỏ 20</label>
          <input type="number" value={editForm.vo20 || ''} onChange={e => setEditForm({ ...editForm, vo20: e.target.value })}
            className="w-full px-3 py-2 bg-[#1e293b] border border-[rgba(255,255,255,0.08)] rounded-lg text-xs text-[#f1f5f9] outline-none focus:border-[#1a56db]" /></div>
        <div><label className="text-[10px] font-medium text-[#94a3b8] mb-1 block">Vỏ 40</label>
          <input type="number" value={editForm.vo40 || ''} onChange={e => setEditForm({ ...editForm, vo40: e.target.value })}
            className="w-full px-3 py-2 bg-[#1e293b] border border-[rgba(255,255,255,0.08)] rounded-lg text-xs text-[#f1f5f9] outline-none focus:border-[#1a56db]" /></div>
        <div><label className="text-[10px] font-medium text-[#94a3b8] mb-1 block">Vỏ 20FR</label>
      ...
      {submission?.history && submission.history.length > 0 && (userRole === 'admin' || userRole === 'super_admin') && (
        <>
          <div className="h-px bg-[rgba(255,255,255,0.08)] my-4" />
